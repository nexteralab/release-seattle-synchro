...
import { uploadNewsCoverImage, calcReadTime, slugify, NEWS_CATEGORIES, type NewsItem } from './services/news.service'
...
interface Props { news?: NewsItem | null }

export function NewsFormPage({ news }: Props) {
  const isEdit = !!news
  ...
  useEffect(() => {
    if (news) {
      ...
      setTags(news.tags ?? [])
      setCoverUrl(news.cover_url ?? null)
  ...
  function goBack() { navigate({ to: '/app/news' }) }
  ...
  async function handleCoverFile(file: File) {
    ...
    try { setCoverUrl(await uploadNewsCoverImage(file)) }
  ...
  async function onSubmit(values: FormValues) {
    const payload = {
      ...
      published,
      published_at: published ? (news?.published_at ?? new Date().toISOString()) : null,
    }
    try {
      if (isEdit && news) {
        await update.mutateAsync({ id: news.id, payload })
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-border bg-background shrink-0 z-20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[13px] text-muted-foreground truncate max-w-[200px]">
            {watch('title') || (isEdit ? 'Edit news' : 'New news')}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button form="news-form" type="submit" size="sm" disabled={isSubmitting}>
            ...
            <div className="max-w-5xl mx-auto px-8 md:px-16 py-10">
              ...
              <Editor
                ...
                onUploadImage={async (file) => ({
                  src: await uploadNewsCoverImage(file),
                  alt: file.name,
                })}
            ...
            <div className="px-5 pb-8 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-muted-foreground text-[12px] shrink-0">/news/</span>
                  <Input
                    {...register('slug')}
              ...
              {/* Category */}
              <div className="space-y-1.5">
                <Label className="text-[11px] tracking-[1.1px] uppercase">Category</Label>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onBlur={addTag}
                  placeholder="synchro, competition..."
              ...
              <div className="border-t border-border pt-4">
                ...
                <div className="bg-card border border-border rounded-md p-3 space-y-0.5">
                  <p className="text-primary text-[11px] truncate">
                    seattlesynchrosst.com/news/{watch('slug') || 'your-news'}
                  </p>
                  <p className="text-[13px] font-medium text-blue-600 dark:text-blue-400 line-clamp-1">
                    {watch('meta_title') || watch('title') || 'News title'}
                  </p>
                  <p className="text-muted-foreground text-[11px] line-clamp-2">
                    {watch('meta_description') || watch('excerpt') || 'News description will appear here.'}
                  </p>
                </div>
              </div>
